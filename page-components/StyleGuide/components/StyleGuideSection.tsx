...
import { transitions } from '@psoares/styled-utils';
...
const ContentWrapper = styled.div<{ elHeight?: number }>`
  ${transitions(['height'], 250, 'inOutCirc')};

  height: ${({ elHeight }: any) => elHeight / 16}rem;
  overflow: hidden;
  position: relative;
  width: 100%;
`;
        ...
        <Row>
          <Col>
            <HeadingWrapper onClick={() => setIsExpanded(!isExpanded)}>
              ...
              <Icon icon={isExpanded ? 'minus' : 'plus'} sz={2} />
            </HeadingWrapper>
          </Col>
        </Row>
        <Row>
          <Col>
            <ContentWrapper elHeight={isExpanded ? contentHeight : 0}>
